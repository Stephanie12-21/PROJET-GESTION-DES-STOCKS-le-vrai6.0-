...
{
    // Utilisez votre propre logique pour générer des valeurs RVB
...
void MainPage2_0::setupChart(QTableView *tableview, int row, int column)
{
    ...
    while (categoryQuery.next()) {
        categories << categoryQuery.value(0).toString(); // L'indice 0 correspond à la colonne "designation"
    }
...
void MainPage2_0::on_PDFProduitbtn_clicked()
{
    ...
    doc.setHtml("<h1>Liste des produits enregistrés</h1>");


    QTextCursor cursor(&doc);


    cursor.insertHtml("<table style=\"border-collapse: collapse;\" border=\"1\">"
                      "<tr>"
                      "<th style=\"border: 1px solid black;\">Numéro du produit</th>"
                      "<th style=\"border: 1px solid black;\">Désignation du produit</th>"
    ...
    while (query.next()) {
        ...
        cursor.insertHtml("<tr>"
                          ...
                          "<td style=\"border: 1px solid black;\">" + numPro + "</td>"
                          "</tr>"
                          "<tr>"
    ...
    QPrinter printer;
    printer.setOutputFileName("D://TEST7.pdf");
...
void MainPage2_0::afficheBonSortie(QTableView *bonSortieTable)
{
    ...
    bonSortieTable->setStyleSheet("QHeaderView::section {background-color:#76D2D1;font-size:11pt;}");
    bonSortieTable->setModel(modeles);
    QFont font;
           font.setPointSize(10);
           ui->bonSortieTable->setFont(font);
...
void MainPage2_0::on_PDFBilanbtn_clicked()
{

    QMessageBox::information(this, "Export en PDF", "L'export en PDF a commencé.");

    QSqlQuery query("SELECT 'BE-' || numBonEntre AS numBon, numPro, qtEntre AS entree, 0 AS sortie, dateEntre AS date "
                    "FROM bonEntree "
                    "UNION ALL "
                    "SELECT 'BS-' || numBonSortie AS numBon, numPro, 0 AS entree, qtSortie AS sortie, dateSortie AS date "
                    "FROM bonSortie "
    ...
    cursor.insertHtml("<table style=\"border-collapse: collapse;\" border=\"1\">"
                      "<tr>"
                      "<th style=\"border: 1px solid black;\">Numéro du bon</th>"
                      "<th style=\"border: 1px solid black;\">Numéro du produit</th>"
                      "<th style=\"border: 1px solid black;\">Entrée</th>"
    ...
    while (query.next()) {
        QString numBon = query.value("numBon").toString();
        QString numPro = query.value("numPro").toString();
        QString qtEntree = query.value("entree").toString();
        QString qtSortie = query.value("sortie").toString();
        QString date = query.value("date").toString();

        cursor.insertHtml("<tr>"
                          "<td style=\"border: 1px solid black;\">" + numBon + "</td>"
                          "<td style=\"border: 1px solid black;\">" + numPro + "</td>"
                          "<td style=\"border: 1px solid black;\">" + qtEntree + "</td>"
                          "<td style=\"border: 1px solid black;\">" + qtSortie + "</td>"
    ...
    QPrinter printer;
    printer.setOutputFileName("D://TESTBilan.pdf");
    printer.setOutputFormat(QPrinter::PdfFormat);
